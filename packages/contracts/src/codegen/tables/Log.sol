...
library Log {
   ...
   * @notice Get the full data.
   */
  function _get(bytes32 key) internal view returns (LogData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   * @notice Set the full data using individual values.
   */
  function _set(bytes32 key, uint8 value) internal {
    bytes memory _staticData = encodeStatic(value);
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes32 key, LogData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.value);
   ...
   */
  function decodeStatic(bytes memory _blob) internal pure returns (uint8 value) {
   ...
   *
   */
  function decode(bytes memory _staticData, PackedCounter, bytes memory) internal pure returns (LogData memory _table) {
